...
- **一份裁决，不是一堆列表** —— 单 IP 一句话结论，逐源证据摆给你看，0-100 置信度（log-odds 贝叶斯融合：源可靠性转对数几率系数、60 天半衰期衰减、交叉佐证；0 = 无证据，而非清白）。
...
> - **A verdict, not a pile of lists** — one line of conclusion per IP, per-source evidence on the table, 0-100 confidence (log-odds Bayesian fusion: reliability → logit coefficients, 60-day half-life decay, corroboration; 0 = no evidence, not innocence).
