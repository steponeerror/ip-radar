...
fixtures(auth_env/auth_client)已提为顶层 tests/conftest.py 共享(Task 3:
管理端点加锁后 tests/core 等目录也要登录)。
"""
import asyncio
...
def test_login_logout_me(auth_client):
    ...
    out = auth_client.post("/api/auth/jwt/logout")
    assert out.status_code == 204
    assert auth_client.get("/api/users/me").status_code == 401


def test_login_wrong_password_envelope(auth_client):
    r = auth_client.post("/api/auth/jwt/login",
